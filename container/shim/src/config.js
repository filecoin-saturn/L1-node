...
export const LOG_INGESTOR_URL = process.env.LOG_INGESTOR_URL || networkToIngestor(SATURN_NETWORK)
...
function networkToOrchestrator (network) {
  switch (network) {
    case 'main': {
...
function networkToIngestor (network) {
  switch (network) {
    case 'main': {
